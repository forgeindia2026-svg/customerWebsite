...
export const DashboardModule: React.FC<DashboardModuleProps> = ({
  ...
  return (
    <div className="space-y-6 text-zinc-900 font-sans">
      ...
      {/* Active Working Job Clean White Card */}
      {activeJob ? (
        <div className="bg-white border border-slate-200/90 rounded-2xl p-5 sm:p-6 shadow-xs space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3.5">
            <div className="flex items-center space-x-2.5">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-ping" />
              <span className="text-[11px] font-mono text-emerald-700 font-bold uppercase tracking-wider bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200">
                ACTIVE JOB IN PROGRESS
              </span>
            </div>
            <span className="text-xs font-mono text-slate-600 font-bold bg-slate-100 border border-slate-200 px-2.5 py-0.5 rounded-md">{activeJob.jobCode}</span>
          </div>

          <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
            <div className="space-y-2">
              <h3 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">{activeJob.title}</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1.5 text-xs text-slate-600">
                <p>
                  <strong className="text-slate-800 font-semibold">Customer:</strong> {activeJob.customer.name} • <span className="text-slate-500">{activeJob.customer.address}</span>
                </p>
                <p>
                  <strong className="text-slate-800 font-semibold">Equipment:</strong> {activeJob.installation.equipmentType}
                </p>
                <p className="sm:col-span-2">
                  <strong className="text-slate-800 font-semibold">Scheduled:</strong> {activeJob.scheduledTimeSlot}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3 flex-shrink-0">
              <button
                ...
                className="px-5 py-2.5 border border-slate-200 bg-slate-50 hover:bg-slate-100 text-slate-800 text-xs font-bold rounded-xl transition-colors cursor-pointer shadow-xs"
              >
                View Details
              </button>
            </div>
