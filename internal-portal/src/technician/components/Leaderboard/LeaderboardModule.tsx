...
export const LeaderboardModule: React.FC<LeaderboardModuleProps> = ({ 
  ...
  return (
    <div className="space-y-4 sm:space-y-6 text-slate-900 font-sans pb-12">
      {/* Top Header & Overview */}
      <div className="bg-white border border-blue-200/80 rounded-2xl p-3.5 sm:p-6 shadow-xs relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 sm:gap-5">
          <div className="space-y-1">
            <div className="flex items-center space-x-2">
              <span className="px-2 py-0.5 rounded-full bg-amber-50 text-amber-800 border border-amber-200/80 text-[9px] sm:text-[10px] font-mono font-bold flex items-center space-x-1">
                <Crown className="w-3 h-3 text-amber-600" />
                <span>FIELD EXCELLENCE</span>
              </span>
              <span className="text-[11px] text-slate-400 font-mono">Live Ranks</span>
            </div>
            ...
            <p className="text-[11px] sm:text-sm text-slate-500 max-w-xl">
          ...
          <div className="flex items-center flex-wrap gap-2">
            ...
            <button
              ...
              className="px-2.5 sm:px-3.5 py-1 sm:py-1.5 text-[11px] sm:text-xs font-bold bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 rounded-xl transition-colors flex items-center space-x-1 cursor-pointer"
            >
        ...
        {myStanding && (
          <div className="mt-3 sm:mt-5 p-2.5 sm:p-4 rounded-xl sm:rounded-2xl bg-gradient-to-r from-blue-50/90 via-indigo-50/70 to-amber-50/60 border border-blue-200/90 flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
            <div className="flex items-center space-x-2.5 sm:space-x-3.5">
              <div className="relative">
                <img 
                  src={myStanding.avatar} 
                  alt={myStanding.name} 
                  className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl object-cover border-2 border-blue-500/80 shadow-xs" 
                />
                <div className="absolute -top-1 -right-1 w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-[9px] sm:text-[11px] font-black flex items-center justify-center border-2 border-white shadow-xs">
                  #{myStanding.rank}
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center space-x-1.5 sm:space-x-2">
                  <h3 className="font-extrabold text-xs sm:text-base text-slate-900">{myStanding.name}</h3>
                  <span className="px-1.5 py-0.2 rounded-md bg-blue-600 text-white text-[8px] sm:text-[10px] font-mono font-black uppercase tracking-wider">
                    YOU
                  </span>
                  <span className="text-[10px] text-slate-500 font-mono">{myStanding.badgeNumber}</span>
                </div>
                <p className="text-[10px] sm:text-xs text-slate-600 flex items-center space-x-1 mt-0.5">
            ...
            <div className="grid grid-cols-3 sm:flex items-center gap-1.5 sm:gap-4">
              <div className="bg-white/90 border border-blue-200/70 p-1.5 sm:px-3.5 sm:py-2 rounded-lg sm:rounded-xl text-center shadow-2xs">
                <span className="text-[8px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-wider block">RANK</span>
                <span className="text-xs sm:text-lg font-black text-blue-700 font-mono">#{myStanding.rank}</span>
              </div>

              <div className="bg-white/90 border border-emerald-200/90 p-1.5 sm:px-3.5 sm:py-2 rounded-lg sm:rounded-xl text-center shadow-2xs">
                <span className="text-[8px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-wider block">EARNINGS</span>
                <span className="text-xs sm:text-lg font-black text-emerald-700 font-mono">
                  ₹{myStanding.earnings.toLocaleString('en-IN')}
                </span>
              </div>

              <div className="bg-white/90 border border-emerald-200/70 p-1.5 sm:px-3.5 sm:py-2 rounded-lg sm:rounded-xl text-center shadow-2xs">
                <span className="text-[8px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-wider block">COMPLETED</span>
                <span className="text-xs sm:text-lg font-black text-emerald-700 font-mono">{myStanding.completedJobs} Jobs</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-3 gap-1.5 sm:gap-6 items-end">
        {/* Rank 2 (Silver - Left) */}
        {top2 && (
          <div className="order-1 md:order-1 bg-gradient-to-b from-white to-slate-50 border-2 border-slate-300/80 rounded-2xl sm:rounded-3xl p-2 sm:p-5 shadow-xs hover:shadow-md transition-all relative overflow-hidden group text-center flex flex-col items-center">
            <div className="w-full bg-slate-200/60 py-0.5 sm:py-1 px-1 sm:px-3 rounded-full text-[8px] sm:text-[10px] font-mono font-extrabold text-slate-700 mb-1 sm:mb-3 flex items-center justify-center space-x-0.5 sm:space-x-1">
              <Medal className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 text-slate-400" />
              <span><span className="sm:hidden">2ND</span><span className="hidden sm:inline">2ND PLACE • SILVER</span></span>
            </div>
            ...
            <div className="relative mb-1 sm:mb-3">
              <img 
                src={top2.avatar} 
                alt={top2.name} 
                className="w-10 h-10 sm:w-20 sm:h-20 rounded-full object-cover border-2 sm:border-4 border-slate-300 shadow-md group-hover:scale-105 transition-transform" 
              />
              <span className="absolute -bottom-1 sm:-bottom-2 inset-x-0 mx-auto w-4 h-4 sm:w-7 sm:h-7 rounded-full bg-slate-400 text-white font-black text-[9px] sm:text-xs flex items-center justify-center shadow-md">
                2
              </span>
            </div>

            <h3 className="font-extrabold text-[11px] sm:text-base text-slate-900 flex items-center justify-center space-x-1 w-full">
              <span className="truncate max-w-[75px] sm:max-w-none">{top2.name}</span>
              {top2.isCurrentUser && (
                <span className="px-1 py-0.2 bg-blue-600 text-white text-[7px] sm:text-[9px] font-bold rounded">YOU</span>
              )}
            </h3>
            <p className="text-[9px] sm:text-xs text-slate-500 font-mono truncate max-w-[75px] sm:max-w-none">{top2.badgeNumber}</p>
            <p className="hidden sm:block text-[11px] text-slate-600 font-medium mt-1">{top2.specialization}</p>

            <div className="mt-1.5 sm:mt-4 w-full grid grid-cols-1 sm:grid-cols-2 gap-1 sm:gap-2 bg-slate-100/80 p-1.5 sm:p-2.5 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-mono">
              <div>
                <span className="text-[8px] sm:text-[10px] text-slate-400 block sm:inline">EARN: </span>
                <span className="font-black text-emerald-700 text-[10px] sm:text-xs">₹{top2.earnings.toLocaleString('en-IN')}</span>
              </div>
        ...
        {/* Rank 1 (Gold MVP Champion - Center Elevated) */}
        {top1 && (
          <div className="order-2 md:order-2 bg-gradient-to-b from-amber-50/90 via-white to-amber-50/50 border-2 border-amber-400/90 rounded-2xl sm:rounded-3xl p-2.5 sm:p-6 shadow-md hover:shadow-lg transition-all relative overflow-hidden group text-center flex flex-col items-center -translate-y-1 sm:-translate-y-2">
            <div className="absolute -top-10 -right-10 w-32 h-32 bg-amber-300/30 rounded-full blur-2xl pointer-events-none" />

            <div className="w-full bg-gradient-to-r from-amber-500 to-amber-600 text-white py-0.5 sm:py-1.5 px-1 sm:px-4 rounded-full text-[8px] sm:text-[11px] font-mono font-black mb-1 sm:mb-3 flex items-center justify-center space-x-0.5 sm:space-x-1.5 shadow-xs">
              <Crown className="w-2.5 h-2.5 sm:w-4 sm:h-4 text-amber-200 fill-amber-200 animate-bounce" />
              <span><span className="sm:hidden">#1 MVP</span><span className="hidden sm:inline">RANK #1 • MVP CHAMPION</span></span>
            </div>

            <div className="relative mb-1 sm:mb-3">
              <div className="w-12 h-12 sm:w-24 sm:h-24 rounded-full p-0.5 sm:p-1 bg-gradient-to-tr from-amber-400 via-yellow-200 to-amber-500 shadow-lg">
                <img 
                  src={top1.avatar} 
                  alt={top1.name} 
                  className="w-full h-full rounded-full object-cover border sm:border-2 border-white group-hover:scale-105 transition-transform" 
                />
              </div>
              <span className="absolute -bottom-1 sm:-bottom-2 inset-x-0 mx-auto w-5 h-5 sm:w-8 sm:h-8 rounded-full bg-amber-500 text-white font-black text-[10px] sm:text-sm flex items-center justify-center border sm:border-2 border-white shadow-md">
                🥇
              </span>
            </div>

            <h3 className="font-black text-xs sm:text-lg text-slate-900 flex items-center justify-center space-x-1 w-full">
              <span className="truncate max-w-[85px] sm:max-w-none">{top1.name}</span>
              {top1.isCurrentUser && (
                <span className="px-1 py-0.2 bg-blue-600 text-white text-[7px] sm:text-[9px] font-bold rounded">YOU</span>
              )}
            </h3>
            <p className="text-[9px] sm:text-xs text-amber-800 font-mono font-bold truncate max-w-[85px] sm:max-w-none">{top1.badgeNumber}</p>
            <p className="hidden sm:block text-xs text-slate-600 font-semibold mt-1">{top1.specialization}</p>

            <div className="mt-1.5 sm:mt-4 w-full grid grid-cols-1 sm:grid-cols-2 gap-1 sm:gap-2 bg-amber-100/60 border border-amber-200/70 p-1.5 sm:p-3 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-mono">
              <div>
                <span className="text-[8px] sm:text-[10px] text-amber-800/80 font-bold block sm:inline">EARN: </span>
                <span className="font-black text-emerald-800 text-[11px] sm:text-lg">₹{top1.earnings.toLocaleString('en-IN')}</span>
              </div>
              <div className="hidden sm:block">
        ...
        {/* Rank 3 (Bronze - Right) */}
        {top3 && (
          <div className="order-3 md:order-3 bg-gradient-to-b from-white to-amber-50/20 border-2 border-amber-600/40 rounded-2xl sm:rounded-3xl p-2 sm:p-5 shadow-xs hover:shadow-md transition-all relative overflow-hidden group text-center flex flex-col items-center">
            <div className="w-full bg-amber-700/10 py-0.5 sm:py-1 px-1 sm:px-3 rounded-full text-[8px] sm:text-[10px] font-mono font-extrabold text-amber-900 mb-1 sm:mb-3 flex items-center justify-center space-x-0.5 sm:space-x-1">
              <Medal className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 text-amber-700" />
              <span><span className="sm:hidden">3RD</span><span className="hidden sm:inline">3RD PLACE • BRONZE</span></span>
            </div>
            ...
            <div className="relative mb-1 sm:mb-3">
              <img 
                src={top3.avatar} 
                alt={top3.name} 
                className="w-10 h-10 sm:w-20 sm:h-20 rounded-full object-cover border-2 sm:border-4 border-amber-600/40 shadow-md group-hover:scale-105 transition-transform" 
              />
              <span className="absolute -bottom-1 sm:-bottom-2 inset-x-0 mx-auto w-4 h-4 sm:w-7 sm:h-7 rounded-full bg-amber-700 text-white font-black text-[9px] sm:text-xs flex items-center justify-center shadow-md">
                3
              </span>
            </div>

            <h3 className="font-extrabold text-[11px] sm:text-base text-slate-900 flex items-center justify-center space-x-1 w-full">
              <span className="truncate max-w-[75px] sm:max-w-none">{top3.name}</span>
              {top3.isCurrentUser && (
                <span className="px-1 py-0.2 bg-blue-600 text-white text-[7px] sm:text-[9px] font-bold rounded">YOU</span>
              )}
            </h3>
            <p className="text-[9px] sm:text-xs text-slate-500 font-mono truncate max-w-[75px] sm:max-w-none">{top3.badgeNumber}</p>
            <p className="hidden sm:block text-[11px] text-slate-600 font-medium mt-1">{top3.specialization}</p>

            <div className="mt-1.5 sm:mt-4 w-full grid grid-cols-1 sm:grid-cols-2 gap-1 sm:gap-2 bg-slate-100/80 p-1.5 sm:p-2.5 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-mono">
              <div>
                <span className="text-[8px] sm:text-[10px] text-slate-400 block sm:inline">EARN: </span>
                <span className="font-black text-emerald-700 text-[10px] sm:text-xs">₹{top3.earnings.toLocaleString('en-IN')}</span>
              </div>
              <div className="hidden sm:block">
                <span className="text-[10px] text-slate-400 block">JOBS</span>
